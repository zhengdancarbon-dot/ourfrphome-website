...
sheets = [
    ...
    Sheet(
        filename="FRP-HOME-200gsm-UD-Carbon-Fiber-Strengthening-Supply-TDS.pdf",
        document_id="FRPH-TDS-UD200-SUPPLY-EN-R01",
        title="200gsm UD Carbon Fiber Strengthening Fabric",
        subtitle="Legacy source-record supply reference for civil strengthening review",
        product_code="FRPH-UD200-SUPPLY / SOURCE RECORD TCU20",
        summary="A 200 g/m2 unidirectional carbon fiber fabric supply reference for directional civil-strengthening review. The archived source identifies the dry fabric form and nominal construction only. Project design, resin compatibility, installation and final order values require separate confirmation.",
        quick=[("DIRECTION", "0 degree UD"), ("WEIGHT", "200 g/m2"), ("THICKNESS", "0.111 mm nominal"), ("COLOR", "Black")],
        tables=[("Source-record product data", ["Property", "Source value", "Quotation / engineering status"], [
            ["Source record", "TCU20", "Legacy product identity; confirm current order code"],
            ["Product form", "Dry unidirectional carbon fiber fabric", "Strengthening-system component, not a cured laminate"],
            ["Primary fiber direction", "0 degree unidirectional", "Final orientation follows the approved design"],
            ["Areal weight", "200 g/m2", "Confirm tolerance, roll and batch documentation"],
            ["Nominal fiber thickness", "0.111 mm", "Dry-fiber reference, not cured-system thickness"],
            ["Color", "Black", "Visual source reference"],
            ["Width and roll length", "Not stated in source record", "Required in RFQ and order specification"],
            ["Fiber grade and sizing", "Not confirmed in visible source record", "Require matching supplier and order documentation"],
            ["Mechanical properties", "Not published in this reference", "Legacy values contained unreadable characters and lacked a complete test basis"],
        ])],
        applications=["Externally bonded civil strengthening subject to engineering design", "Directional surface reinforcement", "Concrete beam, slab, wall or column systems subject to project review", "Composite repair where the selected resin and construction are validated"],
        handling=["A qualified engineer must define the substrate, load direction, reinforcement layout, overlap, anchorage and installation controls.", "Confirm width, roll length, fiber grade, sizing, compatible saturating resin, quantity, destination and final application before quotation.", "Do not use the nominal dry-fiber thickness as cured-system thickness or as a project design allowable.", "Request the final order specification and required batch documents before approval and installation."],
        source_note="Prepared from the archived two-page TCU20 structural-strengthening fabric record. Only clearly readable product fields were retained. Legacy approval language, shelf-life claims, damaged mechanical-property numerals and application assertions were excluded. This document does not establish FRP HOME manufacture, current stock, certification or project approval.",
        issue_date="27 JULY 2026",
        document_label="SUPPLY REFERENCE / DATA SHEET",
    ),
    Sheet(
    ...
    Sheet(
        filename="FRP-HOME-400gsm-Quadraxial-Carbon-NCF-Supply-TDS.pdf",
        document_id="FRPH-TDS-CQX400-EN-R01",
        title="400gsm Quadraxial Carbon NCF",
        subtitle="Supplier-origin 0/+45/90/-45 stitched reinforcement reference",
        product_code="FRPH-CQX400 / SOURCE RECORD ZD QUADRAXIAL 400",
        summary="A four-direction stitched carbon non-crimp fabric offered subject to source availability, approved sample and final order confirmation. The source record distributes 400 g/m2 carbon equally across 0, +45, 90 and -45 degree layers, with 8 g/m2 texturised polyester stitching.",
        quick=[("ARCHITECTURE", "0 / +45 / 90 / -45"), ("CARBON", "400 g/m2"), ("TOTAL DRY", "408 g/m2 +/- 5%"), ("ROLL", "1270 mm x 50 m")],
        tables=[
            ("Supplier-origin construction data", ["Property", "Source value", "Quotation status"], [
                ["Construction", "Quadraxial carbon NCF, 0 / +45 / 90 / -45", "Confirm layer order by approved specification"],
                ["Carbon fiber", "12K standard modulus, 800 tex", "Source reference; confirm grade and sizing"],
                ["Carbon layer distribution", "0: 100; +45: 100; 90: 100; -45: 100 g/m2", "400 g/m2 carbon total"],
                ["Stitching thread", "Texturised polyester, 78 dtex", "Source reference"],
                ["Stitch gauge / length", "6.0 gg / 2.22 mm", "Pillar stitch"],
                ["Stitch weight", "8 g/m2", "Source reference"],
                ["Total dry fabric weight", "408 g/m2 +/- 5%", "Includes stitch yarn"],
                ["Fabric width", "1270 mm +/- 5 mm", "Tapes by application review"],
                ["Roll weight / length", "25.72 kg / 50 m", "Confirm core, packing and usable length"],
            ]),
            ("Order confirmation", ["Item", "Source scope", "Buyer input required"], [
                ["Tracer threads / tapered edges", "Customer specified", "State identification and edge requirements"],
                ["Approved fibers", "By application", "Confirm source grade, sizing and qualification need"],
                ["Cardboard core", "Customer specified", "State core and handling requirements"],
                ["Packaging", "Customer specified", "State roll protection, labels and destination"],
            ]),
        ],
        applications=["Project-defined multidirectional composite laminates", "Process and end use subject to customer validation"],
        handling=["Keep rolls dry, covered and protected from crushing, telescoping and edge damage.", "Confirm layer order, total and per-layer weight, fiber grade, sizing, width, roll format, stitch and packaging before order release.", "Validate drape, compaction, permeability and resin process on representative geometry before production use.", "Treat the source values as nominal supply references, not cured-laminate design allowables or batch certification."],
        source_note="Prepared from the local ZD Quadraxial Carbon Fabric 400gsm source data sheet supplied for website documentation. The source construction values remain identified as supplier-origin references. FRP HOME manufacture, source authorization, stock and batch availability are not implied and must be confirmed separately.",
        issue_date="27 JULY 2026",
        document_label="SUPPLY REFERENCE / DATA SHEET",
    ),
    Sheet(
    ...
    Sheet(
        filename="FRP-HOME-1.2mm-Pultruded-CFRP-Strengthening-Plate-TDS.pdf",
        document_id="FRPH-TDS-CFRPPL12-EN-R01",
        title="1.2 mm Pultruded CFRP Strengthening Plate",
        subtitle="External-bonded carbon laminate for civil structural strengthening",
        product_code="FRPH-CFRP-PLATE-1.2 / source model ZCP1.2",
        summary="A pultruded carbon fiber reinforced polymer laminate for externally bonded civil structural-strengthening systems. Plate selection, substrate preparation, compatible structural epoxy, design actions and installation must be reviewed as one engineered system.",
        quick=[("PROCESS", "Pultruded CFRP"), ("THICKNESS", "1.2 mm"), ("STRENGTH", "2620 MPa typical"), ("MODULUS", "165 GPa typical")],
        tables=[("Source-model reference data", ["Property", "Source value", "Quotation / design status"], [
            ["Source model", "ZCP1.2", "Confirm FRP HOME order code and traceability"],
            ["Product form", "Pultruded CFRP strengthening laminate", "External-bonded system component"],
            ["Color", "Black", "Visual reference"],
            ["Nominal thickness", "1.2 mm", "Width, length and tolerances by quotation"],
            ["Mean tensile strength", "2620 MPa", "Typical source value; test method not stated"],
            ["Mean elastic modulus", "165 GPa", "Typical source value; test method not stated"],
            ["Elongation at break", "1.7%", "Typical source value; test method not stated"],
            ["Fiber volume content", ">68%", "Typical source value"],
            ["Density", "1.6 g/cm3", "Typical source value"],
        ])],
        applications=["Externally bonded civil CFRP strengthening", "Concrete beam and slab reinforcement", "Bridge and building load upgrades", "Timber and masonry strengthening subject to engineering review"],
        handling=["A qualified structural engineer must define the strengthening design, substrate acceptance, anchorage details and installation controls.", "Confirm plate width, length, thickness tolerance, surface preparation, compatible structural epoxy, quantity, destination and final application before quotation.", "Do not use typical source values as project design allowables or as a substitute for project-specific testing, applicable codes and engineering approval.", "Keep plates dry, supported and protected from impact, edge damage, contamination and direct sunlight during storage and transport."],
        source_note="Prepared from the archived ZCP1.2 supplier data sheet dated 12 November 2024. Only explicit source-model values were retained. Legacy approval or certification language and unsupported application-specific assertions were not carried forward. Test methods, tolerances and batch values must be confirmed for the order.",
        issue_date="22 JULY 2026",
    ),
    Sheet(
