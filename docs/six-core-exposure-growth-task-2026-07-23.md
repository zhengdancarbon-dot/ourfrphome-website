...
39. Added the same six-product procurement hierarchy to the English Products directory, before the complete product-family listing. The section reuses the six stable commercial URLs and existing product descriptions, and adds an `ItemList` entity containing names, order and canonical URLs only; no offer, price, inventory, rating or unsupported property was added. Build, 210-URL SEO audit, six-link/schema checks and desktop/mobile QA passed. Evidence is recorded in `docs/six-core-english-products-priority-2026-07-27.md`.
