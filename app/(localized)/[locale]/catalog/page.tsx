...
import {
  createLocalizedCatalogMetadata,
  LocalizedCatalogPage,
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return createLocalizedCatalogMetadata(locale);
}

export default async function LocaleCatalogPage({ params }: PageProps) {
  const { locale } = await params;
  if (!isLocalizedLocale(locale)) notFound();
  return <LocalizedCatalogPage locale={locale as LocalizedLocale} />;
}
